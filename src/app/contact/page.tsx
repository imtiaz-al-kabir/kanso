...
import { MapPin, Phone, Mail, Clock, MessageSquare, Send, Package, Headphones, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-20 font-sans animate-fade-up pb-16">
      ...
      <section className="pt-16 text-center flex flex-col items-center gap-4">
        <span className="text-[10px] tracking-[0.3em] font-bold text-stone-400 uppercase">Get In Touch</span>
        ...
        <p className="text-stone-500 text-sm font-light leading-relaxed max-w-md">
          Whether you have a product question, need delivery support, or want to plan a studio visit in Dhaka — our team is here for you.
        </p>
      </section>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {contactInfo.map((info) => {
          ...
          return (
            <div key={info.title} className="glass-panel rounded-2xl p-6 flex flex-col gap-4 border border-charcoal/5 hover:border-primary/30 transition-colors">
              <div className="w-10 h-10 rounded-xl bg-sand flex items-center justify-center text-charcoal">
                <Icon className="w-5 h-5" />
              </div>
              <div className="flex flex-col gap-1.5">
                <span className="text-[10px] font-bold uppercase tracking-widest text-stone-400">{info.title}</span>
                {info.lines.map((line) => (
                  <span key={line} className="text-xs font-semibold text-charcoal">{line}</span>
                ))}
              </div>
            </div>
          );
      ...
      <section className="grid grid-cols-1 lg:grid-cols-5 gap-12 items-start">
        ...
        <div className="lg:col-span-3 glass-panel rounded-3xl p-8 md:p-10 border border-charcoal/5 flex flex-col gap-7">
          <div className="flex flex-col gap-1.5">
            <h2 className="font-serif text-2xl font-light text-charcoal">Send Us a Message</h2>
            <p className="text-xs text-stone-400 font-light">Average reply time: under 2 hours during studio hours.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs uppercase tracking-wider font-semibold text-stone-500">Your Name *</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs uppercase tracking-wider font-semibold text-stone-500">Phone (Optional)</label>
                <input
                  type="tel"
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs uppercase tracking-wider font-semibold text-stone-500">Your Message *</label>
              <textarea
                rows={5}
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="flex flex-col gap-1.5">
            <span className="text-[10px] font-bold uppercase tracking-widest text-stone-400">Quick Answers</span>
            <h2 className="font-serif text-2xl font-light text-charcoal">FAQ</h2>
          </div>

          <div className="flex flex-col gap-3">
            {faqs.map((faq, i) => (
              <div key={i} className="glass-panel rounded-xl border border-charcoal/5 overflow-hidden">
                <button
                ...
                >
                  ...
                  <span className={`shrink-0 text-lg font-light transition-transform duration-200 ${openFaq === i ? 'rotate-45' : ''}`}>+</span>
                </button>
                {openFaq === i && (
          ...
          <div className="bg-emerald-50 border border-emerald-200/60 rounded-2xl p-5 flex flex-col gap-3">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center text-sm font-bold">W</div>
              <span className="text-xs font-bold text-charcoal uppercase tracking-widest">WhatsApp Us</span>
            </div>
            <p className="text-xs text-stone-600 font-light leading-relaxed">
              For fastest replies, message us directly on WhatsApp — we typically respond within 30 minutes during studio hours.
            </p>
            <a
          ...
          <div className="glass-panel rounded-2xl p-5 flex flex-col gap-3 border border-charcoal/5">
            <div className="flex items-center gap-2 text-charcoal">
              ...
              <span className="text-xs font-bold uppercase tracking-widest">Studio Location</span>
            </div>
            ...
            <span className="text-[10px] text-stone-400 font-semibold">House 23, Road 11, Dhanmondi, Dhaka-1205</span>
          </div>
        </div>
      </section>
      ...
      <section className="glass-panel rounded-3xl p-10 flex flex-col gap-6 border border-charcoal/5">
        <div className="text-center flex flex-col gap-2">
          <span className="text-[10px] tracking-[0.25em] font-bold text-stone-400 uppercase">Payment & Delivery</span>
          <h2 className="font-serif text-2xl font-light text-charcoal">We Make It Easy for You</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {[
            { name: 'bKash', color: 'bg-pink-50 text-pink-600', detail: 'Pay via bKash\ninstantly' },
            { name: 'Nagad', color: 'bg-orange-50 text-orange-600', detail: 'Nagad wallet\naccepted' },
            { name: 'Cash on\nDelivery', color: 'bg-emerald-50 text-emerald-600', detail: 'COD available\nunder ৳5,000' },
            { name: 'Card\nPayment', color: 'bg-blue-50 text-blue-600', detail: 'Visa / MasterCard\nsecure checkout' },
          ].map((m) => (
            <div key={m.name} className={`${m.color} rounded-2xl p-5 flex flex-col gap-2 text-center`}>
              <span className="font-bold text-sm whitespace-pre-line">{m.name}</span>
              <span className="text-[10px] font-light whitespace-pre-line opacity-80">{m.detail}</span>
            </div>
          ))}
        </div>
        <p className="text-center text-xs text-stone-400 font-light">
          Nationwide delivery via Pathao, Steadfast, Sundarban Courier & SA Paribahan. Free delivery on orders above ৳5,000.
        </p>
      </section>
